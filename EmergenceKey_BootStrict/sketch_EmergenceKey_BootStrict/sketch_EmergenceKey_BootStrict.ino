/**
 * EmergenceKey — Boot-Strict variant (keyboard only)
 *
 ...
 *
 * Optional fallback if a BIOS rejects the main (keyboard+mouse) build.
 * Same v1 3-byte wire protocol; mouse frames are parsed and ignored.
 *
 *   - Keyboard only (no USBHIDMouse)
...
// =====================
// OUR IDENTITY (same as the main build / EmergenceKey webpage)
// =====================
...
// =====================
// LED — Waveshare ESP32-S3-Zero onboard WS2812 on GPIO 21 (RGB_BUILTIN).
// GPIO 0 is BOOT.
// =====================
...
typedef enum {
  LED_STATE_IDLE_BREATH,
  LED_STATE_CONNECTED_SOLID,
  LED_STATE_ERROR_BLINK
} led_state_t;
...
// phase clock so a state entered mid-cycle (e.g. a disconnect while
// breathing is 90% through its fade) always gets its own full animation
// window, instead of inheriting stale `elapsed` time from the previous
// state (which could make the error blink last 0 visible frames).
...
// =====================
// Protocol Commands (UNCHANGED — the 3-byte contract survives)
// =====================
...
// Mouse frames stay valid on the wire so the webpage does not need to care
// which sketch is flashed; this build has no mouse interface to send them to.
static void dispatchCmd(uint8_t type, uint8_t a, uint8_t b) {
  switch (type) {
    ...
    case CMD_MOUSE_SCROLL:
      break; // accepted, intentionally ignored — keyboard-only build
    default: break;
  }
}
...
class WriteCallbacks : public NimBLECharacteristicCallbacks {
  void onWrite(NimBLECharacteristic* pCharacteristic, NimBLEConnInfo& connInfo) override {
    ...
    // ---- v1: 3-byte frames (THE contract — unchanged) ----
...
void setup() {
  ...
  // USB first. Device class 0/0/0 is load-bearing for some firmware USB
  // stacks — do not remove without hardware evidence.
